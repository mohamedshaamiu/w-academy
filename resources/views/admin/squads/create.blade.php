<x-app-layout portal="admin" :title="__('admin.squad.create.title')" :header="__('admin.squad.create.title')">
    @php
        $weekdayLabel = fn (int $isoDay) => __('common.weekday.'.($isoDay === 7 ? 0 : $isoDay));
        $selectedDays = old('training_days', []);
    @endphp

    <form method="POST" action="{{ route('admin.squads.store') }}" class="max-w-3xl space-y-6">
        @csrf
        ...
        <div class="rounded-xl border border-navy-100 bg-white p-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <x-text-input id="name_dv" name="name_dv" class="mt-1 block w-full" required :value="old('name_dv')" />
                ...
                <div>
                    ...
                    <x-text-input id="name_en" name="name_en" class="mt-1 block w-full" required :value="old('name_en')" />
                    <x-input-error :messages="$errors->get('name_en')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="age_group" :value="__('squad.fields.age_group')" />
                    <x-text-input id="age_group" name="age_group" class="mt-1 block w-full" required :value="old('age_group')" />
                ...
                <div>
                    ...
                    <select id="head_coach_id" name="head_coach_id" class="mt-1 block w-full rounded-md border-navy-200">
                        ...
                        @foreach ($coaches as $coach)
                            <option value="{{ $coach->id }}" @selected((string) old('head_coach_id') === (string) $coach->id)>{{ $coach->user->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="venue_dv" name="venue_dv" class="mt-1 block w-full" :value="old('venue_dv')" />
                ...
                <div>
                    ...
                    <x-text-input id="venue_en" name="venue_en" class="mt-1 block w-full" :value="old('venue_en')" />
                ...
                <div>
                    ...
                    <x-text-input id="default_start_time" type="time" name="default_start_time" class="mt-1 block w-full" :value="old('default_start_time')" />
                ...
                <div>
                    ...
                    <x-text-input id="default_end_time" type="time" name="default_end_time" class="mt-1 block w-full" :value="old('default_end_time')" />
                    <x-input-error :messages="$errors->get('default_end_time')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="capacity" :value="__('squad.fields.capacity')" />
                    <x-text-input id="capacity" type="number" min="1" name="capacity" class="mt-1 block w-full" :value="old('capacity')" />
