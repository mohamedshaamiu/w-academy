<x-app-layout portal="admin" :title="__('admin.session.create.title')" :header="__('admin.session.create.title')">
    <form method="POST" action="{{ route('admin.sessions.store') }}" class="max-w-3xl space-y-6">
        @csrf
        ...
        <div class="rounded-xl border border-navy-100 bg-white p-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <select id="squad_id" name="squad_id" class="mt-1 block w-full rounded-md border-navy-200" required>
                        ...
                        @foreach ($squads as $squad)
                            <option value="{{ $squad->id }}" @selected((string) old('squad_id') === (string) $squad->id)>{{ $squad->translated('name') }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="coach_id" type="number" min="1" name="coach_id" class="mt-1 block w-full" :value="old('coach_id')" />
                ...
                <div>
                    ...
                    <x-text-input id="scheduled_start" type="datetime-local" name="scheduled_start" class="mt-1 block w-full" required :value="old('scheduled_start')" />
                ...
                <div>
                    ...
                    <x-text-input id="scheduled_end" type="datetime-local" name="scheduled_end" class="mt-1 block w-full" required :value="old('scheduled_end')" />
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full rounded-md border-navy-200">{{ old('notes') }}</textarea>
