<x-app-layout portal="admin" :title="__('admin.agreement.edit.title')" :header="__('admin.agreement.edit.title')">
    <form
        method="POST"
        action="{{ route('admin.agreement-templates.update', $template) }}"
        class="max-w-4xl space-y-6"
        x-data="{
            clauses: {{ Illuminate\Support\Js::from(old('consent_clauses', $template->consent_clauses ?: [['key' => '', 'label_dv' => '', 'label_en' => '', 'required' => true]])) }},
        ...
        <div class="rounded-xl border border-navy-100 bg-white p-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <x-text-input id="version" type="number" min="1" name="version" class="mt-1 block w-full" required :value="old('version', $template->version)" />
                ...
                <div>
                    ...
                    <x-text-input id="effective_from" type="date" name="effective_from" class="mt-1 block w-full" required :value="old('effective_from', $template->effective_from->toDateString())" />
                ...
                <div>
                    ...
                    <x-text-input id="title_dv" name="title_dv" class="mt-1 block w-full" required :value="old('title_dv', $template->title_dv)" />
                ...
                <div>
                    ...
                    <x-text-input id="title_en" name="title_en" class="mt-1 block w-full" required :value="old('title_en', $template->title_en)" />
                    <x-input-error :messages="$errors->get('title_en')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="body_dv" :value="__('admin.agreement.fields.body').' ('.__('common.locale.dv').')'" />
                    <textarea id="body_dv" name="body_dv" rows="8" class="mt-1 block w-full rounded-md border-navy-200" required>{{ old('body_dv', $template->body_dv) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="body_en" name="body_en" rows="8" class="mt-1 block w-full rounded-md border-navy-200" required>{{ old('body_en', $template->body_en) }}</textarea>
